...
class _PageDetailCarteCreditState extends State<PageDetailCarteCredit> {
  ...
  List<TextEditingController> _depensesFixesControllers = [TextEditingController()];
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'Paiement minimum requis (\$)'),
                   keyboardType: const TextInputType.numberWithOptions(decimal: true),
                ),
                TextField(
                  controller: jourPaiementController,
                  decoration: const InputDecoration(labelText: 'Jour du paiement (1-28)'),
          ...
          actions: [
            ElevatedButton(
              onPressed: () {
                ...
                final double limite = double.tryParse(limiteController.text) ?? 0.0;
                final double paiementMin = double.tryParse(paiementMinController.text) ?? 0.0;
                final int jourPaiement = int.tryParse(jourPaiementController.text) ?? 1;
                ...
                if (limite > 0 && paiementMin > 0) {
                  setState(() {
                    ...
                    if (_dateEcheance.isBefore(now)) {
                      _dateEcheance = DateTime(now.year, now.month + 1, jourPaiement);
                    }
                  });
  ...
  void _showUpdateSoldeDialog() {
    final soldeController = TextEditingController(text: _soldeActuel.toStringAsFixed(2));
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: const InputDecoration(labelText: 'Nouveau solde actuel (\$)'),
            keyboardType: const TextInputType.numberWithOptions(decimal: true),
            autofocus: true,
          ),
          actions: [
  ...
  void _ajouterDepenseFixe() {
    setState(() {
      _depensesFixesControllers.add(TextEditingController());
    });
  }

  void _calculerFraisInteretMensuel() {
    final double tauxAnnuel = double.tryParse(_tauxInteretController.text) ?? 0.0;
    if (tauxAnnuel > 0) {
      final double interet = _soldeActuel * (tauxAnnuel / 100 / 12);
      setState(() {
        _fraisInteretMensuel = 'Frais d\'intérêt mensuels estimés: \$${interet.toStringAsFixed(2)}';
      });
    }
  }
  ...
  void _calculerTempsRemboursement() {
    ...
    double paiementMensuel = double.tryParse(_paiementMensuelController.text) ?? 0.0;
    double soldeCourant = _soldeActuel;
    
    // Validation
    if (paiementMensuel <= (soldeCourant * tauxMensuel) + _totalDepensesFixes) {
      setState(() {
        _resultatCalculTemps = 'Le paiement doit être supérieur aux intérêts et aux dépenses fixes.';
      });
      return;
    }
    ...
    while (soldeCourant > 0) {
      if (mois > 1200) { // Sécurité pour éviter une boucle infinie
    ...
    DateTime dateFin = DateTime.now().add(Duration(days: (mois * 30.44).round()));
    setState(() {
      _resultatCalculTemps = 'Payé en: ${DateFormat.yMMM('fr_CA').format(dateFin)} ($mois mois)';
    });
  }
  ...
  void _calculerPaiementRequis() {
    ...
    final int n = _dateCible!.difference(DateTime.now()).inDays ~/ 30; // Nombre de mois
    ...
    for(int j=0; j<10; j++){ // Répéter pour affiner
    ...
    setState(() {
      _resultatCalculPaiement = 'Paiement mensuel requis: \$${paiementEstime.toStringAsFixed(2)}';
    });
  }
  ...
  Widget _buildResumeCarte() {
    ...
    double utilisationPourcentage = _limiteCredit > 0 ? creditUtilise / _limiteCredit : 0;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('\$${_soldeActuel.toStringAsFixed(2)}', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),

            // Barre de progression
            Text('Utilisation du crédit: \$${creditUtilise.toStringAsFixed(2)} / \$${_limiteCredit.toStringAsFixed(2)}'),
            ...
            LinearProgressIndicator(
              ...
              valueColor: AlwaysStoppedAnimation<Color>(utilisationPourcentage > 0.8 ? Colors.red : Colors.blue),
            ),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _buildInfoCarte('Crédit dispo.', '\$${creditDisponible.toStringAsFixed(2)}'),
                _buildInfoCarte('Paiement min.', '\$${_paiementMinimum.toStringAsFixed(2)}'),
                _buildInfoCarte('Prochain paiement', DateFormat.yMd('fr_CA').format(_dateEcheance)),
              ],
            ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Plan de remboursement', style: Theme.of(context).textTheme.headlineSmall),
            const SizedBox(height: 16),
            
            // Entrées
            TextField(
              controller: _tauxInteretController,
              decoration: const InputDecoration(labelText: 'Taux d\'intérêt annuel (%)', border: OutlineInputBorder()),
              keyboardType: const TextInputType.numberWithOptions(decimal: true),
              onChanged: (_) => _calculerFraisInteretMensuel(),
            ),
            const SizedBox(height: 8),
            if (_fraisInteretMensuel.isNotEmpty)
              Text(_fraisInteretMensuel, style: Theme.of(context).textTheme.bodySmall),

            const SizedBox(height: 16),
            Text('Dépenses fixes mensuelles sur la carte', style: Theme.of(context).textTheme.titleSmall),
              ...
              contenu: Column(
                children: [
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Je paie ce montant par mois (\$)', border: OutlineInputBorder()),
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  ),
                  ...
                  if (_resultatCalculTemps.isNotEmpty)
                    Padding(
                      ...
                      child: Text(_resultatCalculTemps, style: Theme.of(context).textTheme.titleMedium?.copyWith(color: Theme.of(context).colorScheme.primary)),
                    ),
                ],
              )
            ),
            ...
            _buildScenarioCard(
              ...
              contenu: Column(
                children: [
                  ListTile(
                    title: Text(_dateCible == null ? 'Choisir une date' : DateFormat.yMMM('fr_CA').format(_dateCible!)),
                  ...
                  if (_resultatCalculPaiement.isNotEmpty)
                    Padding(
                      padding: const EdgeInsets.only(top: 8.0),
                      child: Text(_resultatCalculPaiement, style: Theme.of(context).textTheme.titleMedium?.copyWith(color: Theme.of(context).colorScheme.primary)),
                    ),
                ],
              )
            ),
  ...
  List<Widget> _buildListeDepensesFixes() {
    return List.generate(_depensesFixesControllers.length, (index) {
      return Padding(
        padding: const EdgeInsets.only(top: 8.0),
        child: TextField(
          controller: _depensesFixesControllers[index],
          decoration: const InputDecoration(hintText: 'Ex: Assurance 50\$', border: OutlineInputBorder()),
          keyboardType: const TextInputType.numberWithOptions(decimal: true),
        ),
      );
    });
