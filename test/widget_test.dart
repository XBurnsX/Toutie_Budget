...
void main() {
  ...
  testWidgets('Affichage d\'erreur si champ requis vide', (WidgetTester tester) async {
  ...
  testWidgets('Affichage d\'erreur si transfert entre comptes incompatibles', (WidgetTester tester) async {
            ...
            return Column(
              children: [
                DropdownButton<String>(
                  ...
                  value: compteSource,
                  items: [
                    DropdownMenuItem(value: 'Compte A', child: Text('Compte A')),
                    DropdownMenuItem(value: 'Compte B', child: Text('Compte B')),
                  ],
                ...
                ElevatedButton(
                  onPressed: () {
                    if (compteSource == compteDest) {
                      setState(() => erreur = 'Impossible de transférer vers le même compte');
                    } else if (compteSource != 'Compte A') {
                      setState(() => erreur = 'Compte de provenance invalide');
                    } else {
    ...
    await tester.pump();
    expect(find.text('Impossible de transférer vers le même compte'), findsOneWidget);
  ...
  testWidgets('Affichage d\'erreur pour tous les cas de transfert (compte-compte, enveloppe-enveloppe, enveloppe-compte, etc.)', (WidgetTester tester) async {
    String? erreur;
              ...
              children: [
                DropdownButton<String>(
                  ...
                  items: [
                    ...
                    DropdownMenuItem(value: 'Enveloppe', child: Text('Enveloppe')),
                  ],
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    if (source == dest && sourceType == destType) {
                      setState(() => erreur = 'Impossible de transférer vers la même entité');
                    }
                    // Cas d'erreur : provenance incompatible (ex : enveloppe avec provenance différente)
                    else if (sourceType == 'Enveloppe' && provenance != source) {
                      setState(() => erreur = 'Provenance incompatible avec l\'enveloppe source');
                    }
                    // Cas d'erreur : enveloppe vers enveloppe avec provenances différentes
                    else if (sourceType == 'Enveloppe' && destType == 'Enveloppe' && provenance != dest) {
                      setState(() => erreur = 'Impossible de mélanger des fonds de provenances différentes');
                    }
                    ...
                    // Cas d'erreur : source ou destination archivée (ici simulé par B)
                      ...
                      setState(() => erreur = 'Source ou destination archivée');
                    }
                    else {
                      setState(() => erreur = null);
                    }
    ...
    await tester.tap(find.text('Transférer'));
    await tester.pump();
    expect(find.text('Impossible de transférer vers la même entité'), findsOneWidget);
    ...
    await tester.tap(find.text('Transférer'));
    await tester.pump();
    expect(find.text('Provenance incompatible avec l\'enveloppe source'), findsOneWidget);
    ...
    await tester.tap(find.text('Transférer'));
    await tester.pump();
    expect(find.text('Impossible de mélanger des fonds de provenances différentes'), findsOneWidget);
